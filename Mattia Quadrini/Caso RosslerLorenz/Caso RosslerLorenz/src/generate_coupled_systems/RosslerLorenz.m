function [X,Y,t,info] = RosslerLorenz(C,t_end,N,X0,Y0,method)
...
%% Simulate
% NOTA : Ricreo qui il nuovo sistema
...
if method == "EulerBackward"
    ...
    for i = 2 : N
        ...
        X(i,1) = X(i-1,1) + (-6*(X(i-1,2)+X(i-1,3)))*dt;
        X(i,2) = X(i-1,2) + (6*(X(i-1,1)+0.2*X(i-1,2)))*dt;
        X(i,3) = X(i-1,3) + (6*(0.2+X(i-1,3)*(X(i-1,1)-5.7)))*dt;

        Y(i,1) = Y(i-1,1) + (10*(-Y(i-1,1)+Y(i-1,2)))*dt;
        Y(i,2) = Y(i-1,2) + (28*Y(i-1,1)-Y(i-1,2)-Y(i-1,1)*Y(i-1,3)+C*(X(i-1,2)*X(i-1,2)))*dt;
